...
from torchmeta.utils.data.dataset import CombinationMetaDataset, CombinationMetaDataset_no_shuffle
from torchmeta.utils.data.sampler import (CombinationSequentialSampler,
                                          CombinationRandomSampler, CombinationRandomSampler_no_shuffle)
...
class MetaDataLoader_no_shuffle(DataLoader):
    def __init__(self, dataset, batch_size=1, shuffle=True, sampler=None,
                 batch_sampler=None, num_workers=0, collate_fn=None,
                 pin_memory=False, drop_last=False, timeout=0,
                 worker_init_fn=None):
        ...
        if isinstance(dataset, CombinationMetaDataset) and (sampler is None):
            if shuffle:
                sampler = CombinationRandomSampler_no_shuffle(dataset)
            else:
                sampler = CombinationSequentialSampler_no_shuffle(dataset)
            shuffle = False
        self.identity = "no_shuffle"

        super(MetaDataLoader_no_shuffle, self).__init__(dataset, batch_size=batch_size,
            shuffle=shuffle, sampler=sampler, batch_sampler=batch_sampler,
            num_workers=num_workers, collate_fn=collate_fn,
            pin_memory=pin_memory, drop_last=drop_last, timeout=timeout,
            worker_init_fn=worker_init_fn)
...
class BatchMetaDataLoader_no_shuffle(MetaDataLoader_no_shuffle):
    def __init__(self, dataset, batch_size=1, shuffle=True, sampler=None, num_workers=0,
                 pin_memory=False, drop_last=False, timeout=0, worker_init_fn=None):
        collate_fn = BatchMetaCollate(default_collate)

        super(BatchMetaDataLoader_no_shuffle, self).__init__(dataset,
            batch_size=batch_size, shuffle=shuffle, sampler=sampler,
            batch_sampler=None, num_workers=num_workers,
            collate_fn=collate_fn, pin_memory=pin_memory, drop_last=drop_last,
            timeout=timeout, worker_init_fn=worker_init_fn)
