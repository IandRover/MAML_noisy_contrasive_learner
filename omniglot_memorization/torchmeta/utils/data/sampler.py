...
class CombinationRandomSampler_no_shuffle(RandomSampler):
    def __init__(self, data_source):
        #if not isinstance(data_source, CombinationMetaDataset):
        #    raise TypeError('Expected `data_source` to be an instance of '
        #                    '`CombinationMetaDataset`, but found '
        #                    '{0}'.format(type(data_source)))
        # Temporarily disable the warning if the length of the len	gth of the 
        # dataset exceeds the machine precision. This avoids getting this
        # warning shown with MetaDataLoader, even though MetaDataLoader itself
        # does not use the length of the dataset.
        with warnings.catch_warnings():
            warnings.simplefilter('ignore')
            super(CombinationRandomSampler_no_shuffle, self).__init__(data_source,
                                                           replacement=True)
        self.identity = "sampler no shuffle"
    def __iter__(self):
        num_classes = len(self.data_source.dataset)
        num_classes_per_task = self.data_source.num_classes_per_task
        for _ in combinations(range(num_classes), num_classes_per_task):
#            yield tuple(random.sample(range(num_classes), num_classes_per_task))
            num_per_channel = num_classes//5
#            selected_cls = list(np.random.choice(num_per_channel, self.num_classes_per_task, True))
#            selected_cls = list(random.choices(range(num_per_channel), num_classes_per_task))
            selected_cls = [random.sample(range(num_per_channel), 1)[0] for i in range(num_classes_per_task)]
            index = [cls + index*num_per_channel for index, cls in enumerate(selected_cls)]
            yield tuple(index)